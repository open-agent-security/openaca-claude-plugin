...
**Tech Stack:** Claude Code plugin manifest, Claude Code skills, `uvx --from openaca openaca`, Python 3 standard-library validation.
...
Create a `scan` skill that runs `uvx --from openaca openaca scan endpoint -v`, `uvx --from openaca openaca scan endpoint -v --project .`, or `uvx --from openaca openaca scan repo --target .` depending on the user's requested scope.

- [x] **Step 2: Add Agent BOM skill**

Create a `bom` skill that runs `uvx --from openaca openaca bom endpoint --output openaca-agent-bom.json`, `uvx --from openaca openaca bom endpoint --project . --output openaca-agent-bom.json`, or `uvx --from openaca openaca bom repo --target . --output openaca-agent-bom.json`.
