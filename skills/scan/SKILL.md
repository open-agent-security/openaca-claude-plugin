...
```bash
uvx --prerelease allow --isolated --from openaca openaca scan endpoint -v --include-posture
```
...
```bash
uvx --prerelease allow --isolated --from openaca openaca scan endpoint --project /path/to/project -v --include-posture
```

- For a repository-only scan, run repo scan with the repo path as target:

```bash
uvx --prerelease allow --isolated --from openaca openaca scan repo --target /path/to/repo -v --include-posture
```
