...
```bash
uvx openaca bom endpoint --output openaca-agent-bom.json
```
...
```bash
uvx openaca bom repo --target . --output openaca-agent-bom.json
```
...
```bash
uvx openaca scan bom --input openaca-agent-bom.json
```
